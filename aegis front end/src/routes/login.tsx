...
export const Route = createFileRoute("/login")({
  ...
  head: () => ({
    meta: [
      { title: "Sign in — Aegis" },
      { name: "description", content: "Sign in to your Aegis quantum cryptographic posture dashboard." },
    ],
  }),
  component: LoginPage,
});
...
function LoginPage() {
  ...
  return <AuthShell mode="login" email={email} setEmail={setEmail} password={password} setPassword={setPassword} loading={loading} onSubmit={onSubmit} onGoogle={onGoogle} />;
}
  ...
  return (
    <div className="relative isolate flex min-h-screen items-center justify-center overflow-hidden bg-background px-4">
      ...
      <div className="w-full max-w-md">
        <Link to="/" className="mb-8 flex justify-center"><AegisLogo size={32} /></Link>
        <div className="surface p-8">
          ...
          <button
            ...
            className="mt-6 inline-flex h-10 w-full items-center justify-center gap-2 rounded-md border border-border bg-elevated-2 text-sm font-medium transition-colors hover:border-quantum-cyan/40 hover:bg-elevated"
          >
          ...
          <form onSubmit={props.onSubmit} className="space-y-3">
            ...
            <button
              ...
              className="mt-2 inline-flex h-10 w-full items-center justify-center gap-2 rounded-md bg-quantum text-sm font-medium text-primary-foreground shadow-glow disabled:opacity-60"
            >
          ...
          <p className="mt-5 text-center text-sm text-muted-foreground">
            {isLogin ? (
              <>Don't have an account? <Link to="/signup" className="text-foreground hover:text-quantum-cyan">Sign up</Link></>
            ) : (
              <>Already have an account? <Link to="/login" className="text-foreground hover:text-quantum-cyan">Sign in</Link></>
            )}
          </p>
        </div>
      </div>
...
function GoogleIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 48 48" aria-hidden="true">
      ...
      <path fill="#4CAF50" d="M24 43.5c5 0 9.5-1.9 12.9-5l-6-4.9c-2 1.5-4.5 2.4-6.9 2.4-5.2 0-9.6-3-11.3-7.5l-6.6 5.1C9.5 39 16.2 43.5 24 43.5z" />
      <path fill="#1976D2" d="M43.6 20.5H42V20H24v8h11.3c-.8 2.2-2.3 4.1-4.4 5.4l6 4.9c-.4.4 6.6-4.8 6.6-14.3 0-1.2-.1-2.4-.3-3.5z" />
    </svg>
  );
}
